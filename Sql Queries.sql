use Netflix;
/*Customers from USA in Signup table*/
select * from Signup where Country='USA';

/*Sub Query- Customers who has subscription plan with HD definition*/
select * from Signup where SubscriptionID in (select SubscriptionId from subscription where Definition='HD');

/*Inner join- List all the customers who pay more than 200 for subscription*/
SELECT signup.subscriptionID, signup.CustomerID, Signup.FirstName, Signup.LastName 
FROM Signup 
INNER JOIN subscription ON signup.SubscriptionID=subscription.SubscriptionId where subscription.Price>'200' ;
